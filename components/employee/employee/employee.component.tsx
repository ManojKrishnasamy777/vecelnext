...
const EmployeeListComponent = ({ vendorId }: { vendorId?: any }) => {
    ...
    useEffect(() => {
        const filtered = allRecords.filter((item: any) =>
            ...
            item.employee_role.name.toLowerCase().includes(search.toLowerCase()) ||
    ...
    const fetchEmployeeList = async () => {
        ...
        const res = await CommonService.GetAll(`/v1/Employee/ListByVendorId/${vendorId}`);
        if (res.length > 0) {
            setAllRecords(res);
            setPage(1);
        }
    ...
    const editEmployee = async (id: number) => {
        router.push('/vendor/' + vendorId + '/' + 'employee/' + id);
    };
        ...
        <div className="panel mt-6">
            ...
            <div className="datatables pagination-padding">
                <DataTable
                    ...
                    columns={[
                        { accessor: 'employee_role.name', title: 'Role', sortable: true },
