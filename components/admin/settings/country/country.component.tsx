...
const CountryListComponent = () => {
    ...
    const editCountry = async (id: number) => {
        if (id === 0) {
            setCountryData({});
        } else {
            const res = await CommonService.GetById(id, '/v1/Country/ById');
            if (res) {
                setCountryData(res);
            }
        }
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setCountryData((prev: any) => ({ ...prev, [id]: value }));
    };
    ...
    const Saveorupdate = async () => {
        ...
        let res: any;
        if (countryData.id) {
            res = await CommonService.CommonPut(countryData, `/v1/Country/Update/${countryData.id}`);
        }
        else {
            res = await CommonService.CommonPost(countryData, '/v1/country/Insert');
        }
        if (res.Type == 'S') {
            fetchCountryList();
    ...
    const Delete = async (id: any, name: any) => {
        Swal.fire({
            ...
            if (result.value) {
                let res: any;
                res = await CommonService.CommonDelete(`/v1/Country/Delete/${id}`);
                if (res.Type == "S") {
                    fetchCountryList();
                    CommonHelper.SuccessToaster(res.Message);
                }
    ...
    return (
        <div className="panel mt-6">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                <h2 className="text-xl">Country List</h2>
                <div className="flex w-full flex-col gap-4 sm:w-auto sm:flex-row sm:items-center sm:gap-3">
                    <button type="button" className="btn btn-primary" onClick={() => editCountry(0)}>
                        ...
                        { accessor: 'code', title: 'Code', sortable: true },
                        {
                            ...
                            render: ({ id, name }) => (
                                <div className="mx-auto flex w-max items-center gap-4">
                                    <button type="button" className="flex hover:text-primary" onClick={() => editCountry(id)}>
                            ...
                            >
                                <DialogPanel className="panel w-full max-w-lg overflow-hidden rounded-lg border-0 p-0 text-black dark:text-white-dark">
                                    ...
                                    <div className="bg-[#fbfbfb] py-3 text-lg font-medium ltr:pl-5 ltr:pr-[50px] rtl:pl-[50px] rtl:pr-5 dark:bg-[#121c2c]">
                                        {countryData.id ? 'Edit Country' : 'Add Country'}
                                    </div>
                                    <div className="p-5">
                                        <form>
                                            <div className="mb-5">
                                                ...
                                                /><br></br>
                                                <span className="text-danger">{CountryValidation["name"]}</span>
                                            </div>
                                            <div className="mb-5">
                                                ...
                                                <input
                                                    ...
                                                    onChange={handleInputChange}
                                                /><br></br>
                                                <span className="text-danger">{CountryValidation["code"]}</span>
                                            </div>
